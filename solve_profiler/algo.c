...
int checkIfBackCross(Cube *thiscube) {
    ...
    if (thiscube->back[1][1] == thiscube->back[0][1] &&
        thiscube->back[1][1] == thiscube->back[2][1]) {
        count += 2;
    }

    if (thiscube->back[1][1] == thiscube->back[1][0] &&
        thiscube->back[1][1] == thiscube->back[1][2]) {
        count += 2;
    }
    return count;
}
...
int main()
{
    ...
    randCube(&cube, 52);
    clock_t start = clock();
    // cubeSolve(&cube, foutput, "solution.txt");
    cubeSolve(&cube);
    clock_t end = clock();
    printf("%0.f", (double)(end - start) / (CLOCKS_PER_SEC / 1000));
    // fclose(foutput);
}
